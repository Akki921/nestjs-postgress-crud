...
export class StudentTeacherService {
  ...
  async createStudentTecher(createStudentTeacher: StudentTeacher): Promise<StudentTeacher> {
    Logger.log('createStudentTeacherSchema from servoice', createStudentTeacher);
    const Student = await this.studentRepository.findOne({where: {id: createStudentTeacher.studentId}});
  Logger.log('Student',Student)
    if(!Student){
        throw new NotFoundException()
        }
   const  newStudentTeacher=   await this.studentteacherRepository.save(createStudentTeacher)
